...
from src.llm.coreference import CoreferenceOutput
...
class SlotContext:
    ...
    persisted to Redis for cross-turn coreference
    """
...
class CoreferenceResolver:
    """
    Resolves ambiguous slot references using the last-turn context (SlotContext in Redis).
    
    This class provides methods to:
    - Resolve ambiguous pronouns to concrete slot values
    - Persist and retrieve context between turns
    - Handle coreference resolution for ambiguous references
    """

    REDIS_KEY_PATTERN = "voice:last_service:{sender_id}"
    REDIS_TTL_SECONDS = 40

    _pronoun_map: ClassVar[dict[str, str]] = {
        "它": "Position",
        "那个": "last_entity",
        "继续": "last_incomplete",
        "调高": "last_adjustable",
    ...
    # tier 1 rule-based resolution
    def resolve(
        self,
        pronoun: str, # already the extracted pronoun from current turn
        sender_id: str,
        current_slots: dict[str, SlotValue],
    ) -> dict[str, Any] | None:
        """
        Fetches the stored context from Redis and applies the appropriate strategy.
        This method looks up the last-turn context from Redis, 
        then applies coreference resolution rules to resolve the pronoun 
        to a concrete slot value.
        
        Args:
            pronoun: The ambiguous pronoun or reference word.
            sender_id: Unique user identifier, used as Redis key suffix.
            current_slots: Slots extracted from the current turn.

        Returns:
            Dict mapping slot key to resolved value, or None if unresolved.
        """
        slot_type = self._pronoun_map.get(pronoun) # pronoun from current turn
        ...
        key = self.REDIS_KEY_PATTERN.format(sender_id=sender_id)
        raw = self._redis.get(key)
        if raw is None:
            return None

        # retrieve last turn context from redis using the sender information 
        ctx = SlotContext.from_redis_value(raw)

        if slot_type == "Position":
            pos_slot = ctx.slots.get("Position") # get related slot from last turn
            if pos_slot:
                return {"Position": pos_slot.resolved_value}
        elif slot_type == "last_adjustable":
            for k in ("Number", "Ratio"): 
                slot = ctx.slots.get(k) # get related slot from last turn
                if slot:
                    return {k: slot.resolved_value}
        elif slot_type == "last_entity":
            for v in ctx.slots.values(): # get related slot from last turn
                return {v.key: v.resolved_value}

        return None

    def save_context(self, sender_id: str, ctx: SlotContext) -> None:
        """Persist the current turn's context to Redis for the next turn."""
        if self._redis is None:
            return
        key = self.REDIS_KEY_PATTERN.format(sender_id=sender_id)
        self._redis.set(key, ctx.to_redis_value(), ex=self.REDIS_TTL_SECONDS)

    # tier 1 (resolve) + tier 2 (resolve_with_llm) resolution fallback
    def resolve_with_llm_fallback(
        ...
        """
        Two-tier coreference resolution: try rule-based first, then LLM as fallback.

        Tier-1: fast dict lookup for 5 known pronouns.
        Tier-2: LangChain structured output — called only when Tier-1 returns None.

        Args:
            sender_id: Unique user identifier (Redis key suffix).
            current_query: Raw user query for the current turn.
            current_slots: Slots already extracted from the current turn.
            structured_llm: A LangChain model bound with
                .with_structured_output(CoreferenceOutput).
        """
        pronoun = self._extract_pronoun(current_query)

        # tier 1 fast rule-based resolution
        tier1 = self.resolve(
            pronoun=pronoun,
            sender_id=sender_id,
    ...
    def _resolve_with_llm(
        ...
        """
        Invoke the LLM to resolve ambiguous
        ...
        schema = get_shared_slot_schema()

        # we want to relate the current query to the conversation history to resolve the ambiguous references
        prompt = build_coreference_prompt( # build coreference resolution prompt
            slot_definitions=format_slot_definitions(schema), # format slot definition of prompt
            conversation_history=format_conversation_history(history, max_turns=3), # format conversation history of prompt
            current_query=current_query,
            history_turns=3,
        )

        try:
            output: CoreferenceOutput = structured_llm.invoke(prompt)
            resolved = output.resolved_slots
        except Exception:
    ...
    def _load_history(self, sender_id: str, last_n_turns: int = 3) -> list[SlotContext]:
        ...
        for i in range(last_n_turns):
            key = f"{key_prefix}:history:{i}"
            raw = self._redis.get(key)
            if raw:
                try:
                    ctx = SlotContext.from_redis_value(raw)
                    history.insert(0, ctx)
                except Exception:
                    pass
        return history

    def _extract_pronoun(self, query: str) -> str | None:
        """Return the first known pronoun found in the query, or None."""
...
class SharedSlotSchema:
    ...
    @property
    def coreference_rules(self) -> dict[str, Any]:
        return self._data["coreference_resolution"]
