...
|------------|-----------|-----------|-----------------|
| Go_POI | go_poi | `{"poi": "...", "search_mode": "nearby\|city", "city": "...", "poi_type": "...", "index": N}` | 导航搜索 |
| Set_Home | set_frequent_location | `{"location_type": "home", "poi": "..."}` | 设置家地址 |
| Set_Company | set_frequent_location | `{"location_type": "company", "poi": "..."}` | 设置公司地址 |
| Check_Frequent_Location | check_frequent_location | `{"location_type": "home\|company"}` | 查询家/公司地址 |
| Delete_Frequent_Location | delete_frequent_location | `{"location_type": "home\|company"}` | 删除家/公司地址 |
| Collect_Location | collect_location | `{"poi": "..."}` | 收藏当前位置 |
| List_Collected_Locations | list_collected_locations | `{}` | 查看收藏列表 |
| Delete_Collected_Locations | delete_collected_locations | `{"location_id": N}` | 删除收藏地点 |
| Open_Nav | open_nav | `{}` | 打开导航 |
| Close_Nav | close_nav | `{}` | 关闭导航 |
| Home_Condition | home_condition | `{}` | 家路况 |
| Go_Home | go_home | `{}` | 导航回家 | home address required, calculates route, returns distance/ETA |
| Go_Company | go_company | `{}` | 导航公司 | company address required, calculates route, returns distance/ETA |
| Company_Condition | company_condition | `{}` | 公司路况 |
| Check_Area_Traffic | check_area_traffic | `{"poi": "...", "radius": N}` | 周边路况 |
| Check_Route_Traffic | check_route_traffic | `{"destination": "..."}` | 沿途路况 |
| Check_City_Traffic | check_city_traffic_overview | `{"city": "..."}` | 全城路况 |
| Add_Via | add_via | `{"poi": "...", "via_index": N}` | 添加途经点 |
...
| Collect_Current_Location | collect_location | `{}` | 收藏当前位置 |
| Ask_Where_Am_I | ask_where_am_i | `{}` | 当前位置查询 |
...
### search_mode
```
value_range: "nearby" | "city"
resolution: Use "nearby" when user mentions nearby/closest (附近的, 最近的). Use "city" for specific locations or city-wide search.
agents: Navigation Agent
```
...
### radius
```
value_range: positive integer (meters)
resolution: Search radius for nearby queries. Default 1000m for traffic, 5000m for POI search.
agents: Navigation Agent
```

### location_type
```
value_range: "home" | "company"
resolution: Distinguishes between home and company address settings.
agents: Navigation Agent
```
...
| Query Type | Tool | Description |
|-----------|------|-------------|
| 周边路况 | check_area_traffic | Traffic conditions around a POI or current location |
| 沿途路况 | check_route_traffic | Traffic along route to a specific destination |
| 全城路况 | check_city_traffic_overview | City-wide traffic congestion overview |
| 家/公司路况 | home_condition / company_condition | Traffic on route to saved address |
...
{
  "status": "success",
  "action": "go_home" | "go_company",
  "destination": "家" | "公司" | "<custom name>",
  "destination_address": "完整地址",
  "distance": "15.6公里",
  "duration": "25分钟",
  "strategy": "速度优先",
  "message": "正在导航回家，距离15.6公里，预计25分钟"
}
```

**Error Responses:**
- `{"status": "error", "message": "无法获取车辆标识"}` - Vehicle ID not available
- `{"status": "error", "message": "无法获取当前位置，请确保GPS已开启"}` - GPS not available
- `{"status": "error", "message": "尚未设置家/公司地址，请先设置"}` - Home/company not set
- `{"status": "error", "message": "家/公司地址缺少坐标信息，请重新设置"}` - Address missing coordinates
- `{"status": "error", "message": "路线规划失败，请检查网络或稍后重试"}` - Route calculation failed
